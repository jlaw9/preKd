""" TODO rename this file to the name of the package
"""
...
class SingleModel:
    ...
    def __init__(
        ...
        self.atom_features = nfp.preprocessing.features.atom_features_v1
        self.bond_features = nfp.preprocessing.features.bond_features_v1
...
#
#    @property
#    def df_solvent_features_scaled(self) -> pd.DataFrame:
#        if not self.data_scaler:
#            return self.df_solvent_features
#        else:
#            if self.df_solvent_features is None:
#                return None
#            df = self._scale_data(self.df_solvent_features, 
#                                  scaler=self.data_scaler_solvents,
#                                  columns=self.solvent_feature_cols,
#                                  offset=self.scaler_offset,
#                                  )
#            return df
...
#            self.data_scaler_solvents.fit(self.df_solvent_features[self.solvent_feature_cols].values)
    ...
    def generate_preprocessor(
        ...
        **kwargs,
        ...
        """
        # TODO save the preprocessor and such more regularly, not only at the very end of training
    ...
    def _scale_data(self, df, scaler=None, columns=None, offset_factor=0):
        """Scale (prediction or feature) columns of a dataframe.
        
        The RobustScaler will center the the data around 0 (with some values at the median given 0).
        This could be problematic for features where we mask 0s, so an offset can be given
        """
        scaler = self.data_scaler if scaler is None else scaler
        columns = self.prediction_columns if columns is None else columns
        df = df.copy()
        df_scaled = scaler.transform(df[columns].values) + offset_factor
...
class MultiModel:
    ...
    @classmethod
    def load_models(
        cls, folder: Union[Path, str], custom_objects=None, nmodels: Union[int,list]=None
    ) -> MultiModel:
    ...
    #     # Required columns
    #     groupby_columns = ["smiles_compound"]
    ...
    def split_data(
        ...
        ----------
        kfolds : int, optional
            The number of kfolds, must be greater than 1, by default 2
